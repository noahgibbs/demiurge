...
class WorldReloadTest < Minitest::Test
  ...
  def test_dsl_actions_with_middle_world_restore
    ...
    # Reload in the middle of all this...
    engine.reload_from_dsl_text(["World Reload DSL", DSL_TEXT])
    engine.flush_notifications
  ...
  def test_intentions_and_notifications_across_world_restore
    ...
    # Reload to check if on_intention and on_notification keep working
    engine.reload_from_dsl_text(["World Reload DSL", DSL_TEXT])
    engine.flush_notifications
    ...
    # Reload to check if subscribe_to_notifications keeps working
    engine.reload_from_dsl_text(["World Reload DSL", DSL_TEXT])
    engine.flush_notifications
    ...
    # Reload to check if the queued action still happens...
    engine.reload_from_dsl_text(["World Reload DSL", DSL_TEXT])
    engine.flush_notifications
    ...
    # Reload to check if everything still works
    engine.reload_from_dsl_text(["World Reload DSL", DSL_TEXT])
    engine.flush_notifications
